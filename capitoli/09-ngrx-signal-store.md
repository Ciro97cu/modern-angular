...
In alternativa alla type assertion si può tipizzare esplicitamente lo stato con `withState<FlightSearchState>({...})`. Il libro preferisce la prima opzione quando non serve un tipo separato altrove.

**`withProps`** aggiunge proprietà qualunque allo store (anche non-signal: service iniettati, `Subject`/`Observable` per eventi). Riceve una funzione che riceve lo store e ritorna le proprietà. Il prefisso `_` rende il membro **privato**: non è solo convenzione, il type system del NgRx esclude i membri `_*` dal tipo esposto ai consumer (vale per tutte le features).
...
> [!tip] Take-away
> La **stessa feature può comparire più volte** (utile per proprietà che dipendono da altre già definite). In alternativa, una sola funzione con `return` esplicito che dichiara più proprietà insieme.

**`withResource`** (community) integra la [[resource|Resource API]]: la funzione proietta lo store in un oggetto di resource. Non importa quale resource (`httpResource`, `rxResource`): qui la crea `FlightClient.findResource(from, to)` passando i signal — quando cambiano, la resource si ri-triggera.
...
`errorHandling` cambia il comportamento di default (leggere `value` in errore lancia): `'previous value'` mantiene l'ultimo valore, `'undefined value'` mette `undefined` (e lo aggiunge al tipo), `'native'` = comportamento Angular. Il nome della resource fa da prefisso ai membri generati: `flightsValue`, `flightsIsLoading`, `flightsError`, `flightsStatus`, `flightsHasValue()`, `_flightsReload()`.
...
**`withMethods`** definisce metodi che aggiornano lo stato in modo controllato. Le modifiche passano per `patchState(store, ...)`, chiamabile solo dentro lo store, con un **partial state** o una **updater function** `(state) => partial`. Lo stato va trattato come **immutabile** (nuovo oggetto, mai mutazione in place).

```ts
withMethods((store) => ({
  updateFilter(from: string, to: string): void {
    patchState(store, { from, to });               // partial: triggera la resource
  },
  ...
  reload(): void { store._flightsReload(); },
  delay(): void {
...
**`withHooks`** registra funzioni del ciclo di vita: `onInit` (init dello store, es. caricamento iniziale) e `onDestroy` (cleanup).
...
**Disabilitarli in produzione.** Via `isDevMode()` si sceglie a runtime tra `withDevtools` reale e lo stub `withDevToolsStub` (stessa API, no-op):
...
```ts
// flight-detail-store.ts
withMutations((store) => ({
  saveFlight: httpMutation<Flight, Flight>({   // <param, return>
    request: (flight: Flight) => ({
      url: `[...]/flight/${flight.id}`,
      method: 'PUT',
      body: flight,
    }),
    operator: concatOp,                        // semantica chiamate sovrapposte
    onSuccess(result, param) { store._snackBar.open('Flight updated successfully', 'OK', { duration: 3000 }); },
    onError(error, param)   { store._snackBar.open('Failed to update flight', 'OK', { duration: 5000 }); },
  }),
})),
```

I due type parameter sono tipo dell'argomento e tipo del valore di ritorno. La mutation aggiunge allo store il metodo `saveFlight` **più** i signal di stato `saveFlightIsPending` e `saveFlightError`, senza boilerplate. L'`operator` riflette gli operatori di flattening RxJS:

- `switchOp` — annulla la chiamata precedente (conta solo l'ultima).
- `mergeOp` — tutte in parallelo, indipendenti.
- `concatOp` — accoda ed esegue una alla volta. **Default**: preserva l'ordine, niente race condition, niente cancellazioni.
- `exhaustOp` — ignora le nuove finché una è in corso (anti doppio-submit).

> [!tip] Take-away
> Come per la resource, conviene spostare i dettagli di protocollo nel `FlightClient`: un `createSaveMutation(options)` che fa spread di `options` (con `onSuccess`/`onError`) nella `httpMutation`. Lo store delega: `saveFlight: store._flightClient.createSaveMutation({ onSuccess, onError })`.

**Consumare la mutation.** Il metodo ritorna una `Promise` con il risultato in uno di tre stati — `success` (contiene il valore salvato), `error` (oggetto errore), `cancelled` (possibile con `switchOp`/`exhaustOp`).

```ts
export class FlightEdit {
  private readonly store = inject(FlightDetailStore);
  protected readonly isPending = this.store.saveFlightIsPending;  // da withMutations
...
**`rxMutation`.** Identico dal punto di vista di store e componente (stessi metodo e signal di stato), ma invece di `request` riceve una `operation` che ritorna un `Observable`:

```ts
createSaveRxMutation(options: Partial<RxMutationOptions<Flight, Flight>>) {
  return rxMutation({
    ...options,
    operation: (flight: Flight) => this.update(flight),   // delega a HttpClient → Observable
    operator: concatOp,
  });
}
```
...
Esempio realistico nel `PassengerStore`: `updateFilter` come `rxMethod` che fa `patchState` (loading), poi `switchMap` verso il client, con `tap`/`catchError` (o il comodo `tapResponse` da `@ngrx/operators`).

```ts
updateFilter: rxMethod<PassengerFilter>(
  pipe(
    tap((filter) => patchState(store, { name: filter.name, firstName: filter.firstName, isLoading: true, error: null })),
    switchMap((filter) =>
...
// nel componente: this.store.connectFlightId(this.id);  // id = InputSignal da withComponentInputBinding
```
  ...
  withMethods((store) => ({
    load(): void {
      ...
      store._ticketClient.find().subscribe({
        next: (flights) => patchState(store, setAllEntities(flights), { isLoading: false }), // più updater insieme
...
**ID.** Di default gli updater assumono una proprietà `id`. Se l'identificatore ha altro nome, si specifica `selectId`; meglio ancora in una `entityConfig` riusabile:
...
> [!tip] Take-away
> Spesso è meglio **splittare le collection in store separati** (uno store per tipo di entità per feature, più magari uno per la UI state) anziché ammassarle: con gli store leggeri come SignalStore è la prassi e tiene gli store piccoli e a responsabilità distinta.

**Normalizzazione.** Mettere strutture annidate dal backend direttamente nello store porta a **duplicati** (lo stesso passeggero su più voli) e stati incoerenti. Come nei DB relazionali, si normalizza: ogni tipo di entità nella sua collection, i riferimenti solo via ID. Una sola istanza per entità, joinabile per ogni vista via `computed`.
...
> [!info] Mental model
> Quando una feature usa più store, o uno store serve più feature, crescono complessità, rischio di incoerenze e cicli difficili da debuggare. Invece di chiamare i metodi degli store, i componenti **dispatchano eventi**. Gli store mettono su **reducer** che dicono come aggiornare lo stato in reazione a eventi specifici: se l'evento li riguarda reagiscono, altrimenti lo ignorano. I componenti non conoscono la struttura interna degli store. I **reducer sono sempre sincroni**; per side effect e async ci sono gli **event handler** (gli "effects" dell'NgRx Global, rinominati per non confonderli con l'`effect` di Angular): Observable che ricevono eventi e ne pubblicano altri col risultato — un ponte tra eventi. Poiché con SignalStore ci sono **più store** (non uno globale come in Redux), il pattern prevede un **dispatcher** che inoltra gli eventi a tutti gli store.
...
Pattern tipico per le operazioni async: tre eventi (trigger / success / error). Il team NgRx consiglia un `source` **fine-grained** (componente/service/store, es. `luggageOverviewEvents` con source `LuggageOverview` per il trigger, `luggageApiEvents` con source `LuggageApi` per gli esiti), così si vede subito chi scatena cosa.

**Reducer** — feature `withReducer`; `on(event, fn)` collega un evento al reducer, che riceve il payload (tipizzato) e ritorna lo stato (partial, patchato).
...
> [!tip] Take-away
> Per cambiare lo stato, meglio **non** aggiungere metodi con `withMethods` (rischio di conflitti tra features): si forniscono **updater** chiamabili con `patchState`, che incapsulano la struttura interna.
...
**Usare** una custom feature = chiamare la factory nel `signalStore`. I suoi membri (proprietà, metodi, updater) sono poi disponibili nello store e ai consumer.
...
6. Come sono memorizzate internamente le entità di `withEntities` (`entityMap`, `ids`, `entities`)? Cosa fa `selectId`/`collection`? Perché normalizzare?
...
- Per i side effect: **Mutations** (write con stati pending/error automatici) e **reactive methods** (`rxMethod` per la potenza RxJS, `signalMethod` per funzioni pure ri-eseguite) — attenzione a injection context e chiamate sovrapposte.
