...
pagine: "411-422"
tags: [tipo/capitolo, ssr, performance]
---
# 17 · Defer, SSR & Hydration
> 📖 cap.17 · pp.411-422 — *Modern Angular* v1.0.4
...
export const appConfig: ApplicationConfig = {
  providers: [
    provideHttpClient(
      withFetch()                 // in SSR/hydration: usa la fetch API nativa
    ),
    provideClientHydration(
      withIncrementalHydration(), // hydration on-demand
      withEventReplay()           // replay degli eventi pre-hydration
    ),
  ],
};
```
...
> [!tip] Take-away
> In scenari SSR/hydration è raccomandato configurare `HttpClient` con `withFetch()` (fetch API nativa). `provideClientHydration` abilita la full hydration; aggiungendo `withIncrementalHydration()` e `withEventReplay()` si attivano hydration incrementale ed event replay.
